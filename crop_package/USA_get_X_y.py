...
def get_X_y(country_code='USA'):
    ...
    # 0. import df
    X_data = np.load(f'../data/USA_all_states_data_MOD09A1.npz')  #8states_data_both_sat
    X = X_data['X']

    print(X.shape)  #  NEW SHAPE SHOULD BE: 18,8,46,30,13

    df_y = pd.read_csv(f'../raw_data/Crop yield data/COUNTY_level_annual/corn_usa.csv')
