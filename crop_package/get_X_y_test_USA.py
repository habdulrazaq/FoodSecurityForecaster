...
def get_X_y(country_code='USA'):
    ...
    print([name for name in X_data['county_names'] if name in df_y['STATE'].str.strip()])
    exit(0)
    df_y = df_y.sort_values('STATE').sort_values('YEAR', kind='stable')
    print(list(df_y['STATE'].drop_duplicates()))
    for i, (year, year_group) in enumerate(df_y.groupby('YEAR')):
        print(year_group['STATE'].nunique())
